...
export const Placeholder = ({ type }: { type: 'footer' | 'normal' }): JSX.Element => {
    ...
    const verifyCode = async () => {
        try {
            ...
            if (response.data.valid) {
                Router.push(`/daftar?wa=+62${ifFirstCharIsZero(phoneNum)}`);
    ...
    const renderPlaceholder = () => {
        if (otpPlaceholder) {
            return (
                <>
                    <Back cipher={cipher} setCipher={setCipher} onClick={verifyCode} err={err} />
                    {renderError()}
                    <p
                        className={`m-0 mt-2 ${
                            type === 'footer'
                    ...
                    >
                        Gadapet? Coba lagi dalam <span className="inline">{renderCounter()}</span>
                    </p>
                    <p
                        className={`m-0 ${
                            type === 'footer'
    ...
    return (
        <div className="max-w-[500px] mx-auto text-left select-none w-full h-fit" id="placeholder">
            {renderPlaceholder()}
        </div>
    );
};
...
const Front = ({
    ...
    return (
        <div className="flex flex-row justify-between border-4 border-outline w-full h-fit rounded-2xl p-4 sm:text-2xl xs:text-lg text-base shadow-2xl items-center bg-onPrimary focus-within:border-primary fade-in">
            ...
            <RegisterButton
                onClick={onClick}
                disabled={!phoneNum || phoneNum.length < 9 || !/^08\d|^8\d/g.test(phoneNum)}
            />
        </div>
    );
};
