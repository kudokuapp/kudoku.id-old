                ...
                <div className={styles.slides}>
                    <div id="slide-1">
                        <div className={`${styles.mobileIntelligent} h-full overflow-hidden`}>
                            <h5 className="gradient-text-new font-bold text-center text-2xl">
                                Intelligent money tracker
                            </h5>
                            <h2 className="font-bold text-onPrimaryContainer text-center text-3xl md:text-4xl">
                                Dapetin semua informasi transaksi lo, dari seluruh akun keuangan
                                yang lo punya.
                            </h2>
                            <div className={`${styles.intelligentSwiperContainer} h-full py-2`}>
                                <ul className={`${styles.intelligentSwiperIndicator} flex-col`}>
                                    {items.map((_item: any, idx: any) => (
                                        <li
                                            key={idx}
                                            className={`${
                                                styles.intelligentSwiperIndicatorItemLarge
                                            } flex flex-col font-semibold my-4 text-2xl md:text-4xl ${
                                                currentIdx === idx ? styles.active : ''
                                            }`}
                                            onClick={() => indicatorOnClick(idx)}
                                            data-testid="indicator"
                                        >
                                            <span className="text-lg">{_item.imageAlt}</span>
                        ...
                        <div className={`${styles.largeIntelligent} flex flex-col h-full`}>
                            ...
                            <h2 className="font-bold text-onPrimaryContainer text-center text-3xl md:text-4xl">
                                Dapetin semua informasi transaksi lo,
                                <br />
                                dari seluruh akun keuangan yang lo punya.
                            </h2>
                            <div
                                className={`${styles.intelligentSwiperContainer} flex flex-row justify-between w-full mx-8 h-full py-20 xl:py-14`}
                            >
                                <ul
                                    className={`${styles.intelligentSwiperIndicator} mr-10 flex-col w-1/2`}
                                >
                                    {items.map((_item: any, idx: any) => (
                                        <li
                                            key={idx}
                                ...
                                >
                                    {items.map((item: any, idx: any) => (
                                        <div
                                            key={idx}
                                            className={`${
                                                currentIdx === idx ? styles.fadeIn : styles.fadeOut
                                            }`}
                                        >
                                            <Item {...item} />
                                        </div>
